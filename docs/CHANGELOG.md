...
- [文档] 本次仅澄清调度文档边界，并未实现 GitHub Actions 下 `SCHEDULE_TIME` 的运行时触发控制；`SCHEDULE_TIME` 仍仅影响本地内置调度（`python main.py --schedule`）。GitHub Actions 的触发时刻仍由 `00-daily-analysis.yml` 固定 `cron` 决定（周一到周五 10:00 UTC / 北京18:00）；如需调整请修改 workflow cron。本条目为 docs-only 说明，不属于该 Issue 的运行时交付闭环；`#1497` 的 Actions 调度能力补齐仍待实现，请使用 `Refs #1497` 跟踪。
